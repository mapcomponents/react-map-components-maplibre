 ...
 *
 */
const useCameraFollowPath = (props: useCameraFollowPath) => {
	...
	function play() {
		...
		if (!pause.current) {
			...
			var alongRoutelati: any = turf.along(props.route, step.current * kmPerStep).geometry
				.coordinates;
			var alongRoutelong: any = {
			...
			if (step.current * kmPerStep < routeDistance) {
				mapHook.map.map.panTo(alongRoutelati, alongRoutelong);
