...
// super messy draft version that actually works
const createPdf: Function = (
	...
	const bboxCamera = renderMap._cameraForBoxAndBearing(
		...
		[options.bboxUnrotated[2], options.bboxUnrotated[3]],0	);
	let geometryCamera = renderMap._cameraForBoxAndBearing(
		_previewGeojson.geometry.coordinates[0][0],
		_previewGeojson.geometry.coordinates[0][2],
		options.bearing
	);
	console.log(geometryCamera, bboxCamera)
	geometryCamera.zoom = bboxCamera.zoom;
	console.log(geometryCamera)
	//	renderMap.fitBounds(
	//		[_previewGeojson.geometry.coordinates[0][0], _previewGeojson.geometry.coordinates[0][2]],
	//
	//		{ bearing: options.bearing, padding: {left:0, right:0,top:0,bottom:0} }
	//	);
