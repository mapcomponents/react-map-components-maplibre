	...
	_geojsonInfo.line.geometry.coordinates.forEach((coordinate: number, index: number) => {
		...
		if (_geojsonInfo.line.geometry.coordinates[index + 1] && Array.isArray(coordinate)) {
			...
			const wayLength = distance(
				...
				[
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index + 1][0],
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index + 1][1],
				],
				{ units: 'kilometers' }
			);
			...
			for (let i = 0; i < listLength; i++) {
				const x = lerp(
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index][0],
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index + 1][0],
					i / listLength
				);
				const y = lerp(
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index][1],
					(_geojsonInfo.line.geometry.coordinates as coordinateObject)[index + 1][1],
					i / listLength
				);
				const z = lerp(
