...
const MlDraggableFeatureLayer = (props) => {
  ...
  useEffect(() => {
    ...
    if (!mapContext.map.getSource("point")) {
      mapContext.map.loadImage(props.iconSrc,
    ...
    mapContext.map.on("mouseenter", "DraggableZone", function() {
      mapContext.map.setPaintProperty("DraggableZone", "circle-color", "rgba(59,178,208,0.5)");
    ...
    mapContext.map.on("mouseleave", "DraggableZone", function() {
      mapContext.map.setPaintProperty("DraggableZone", "circle-color", "rgba(56,135,190,0.5)");
