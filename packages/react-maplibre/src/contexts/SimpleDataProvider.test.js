	...
	return (
		<>
			<div className="data" data-testid="data">
				{JSON.stringify(simpleDataContext.data)}
			</div>
		</>
	);
};
...
const xmlMockData = new DOMParser().parseFromString(
	'<?xml version="1.0" encoding="UTF-8" ?><records>	<record>		<id>1</id>		<title>Macey, Josephine, Robert, Clark</title>	</record>	<record>		<id>2</id>		<title>Summer, Channing, Colette, Josephine</title>	</record></records>',
	'text/xml'
);
...
describe('SimpleDataProvider', () => {
	it('should retrieve data and provide it through simpleDataContext.data (format: "csv")', async () => {
		...
		render(
			<SimpleDataProvider format="csv" url="test">
				<SimpleDataContextTestComponent />
			</SimpleDataProvider>
		);
		...
		await waitFor(() =>
			expect(screen.getByTestId('data').textContent).toEqual(JSON.stringify(mockData))
		);
	});
	...
	it('should retrieve data and provide it through simpleDataContext.data (format: "xml")', async () => {
		...
		render(
			<SimpleDataProvider
				...
				formatData={(record) => {
					return {
						id: record.querySelector('id').innerHTML,
						title: record.querySelector('title').innerHTML,
					};
				}}
				url="test"
			>
		...
		await waitFor(() =>
			expect(screen.getByTestId('data').textContent).toEqual(
				'[{"id":"1","title":"Macey, Josephine, Robert, Clark"},{"id":"2","title":"Summer, Channing, Colette, Josephine"}]'
			)
		);
	});
	...
	it('should retrieve data and provide it through simpleDataContext.data (format: "json")', async () => {
		...
		render(
			<SimpleDataProvider format="json" url="test">
				<SimpleDataContextTestComponent />
			</SimpleDataProvider>
		);
	...
	it('should retrieve data and provide the content of the key props.dataProperty on the retrieved data through simpleDataContext.data (format: "json")', async () => {
		...
		render(
			<SimpleDataProvider format="json" data_property="data" url="test">
				<SimpleDataContextTestComponent />
			</SimpleDataProvider>
		);
